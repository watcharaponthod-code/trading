...
export async function insertTrade(trade: any): Promise<any> {
  ...
  if (isPostgres) {
    ...
    const info = (db as Database.Database).prepare(`
      INSERT INTO trades (symbol, side, qty, price, order_id, status, strategy_id, signal_reason, confidence, executed_at)
      VALUES (@symbol, @side, @qty, @price, @order_id, @status, @strategy_id, @signal_reason, @confidence, datetime('now'))
    `).run(trade)
    return (db as Database.Database).prepare("SELECT * FROM trades WHERE id = ?").get(info.lastInsertRowid)
  }
}
...
export async function getTrades(limit = 50): Promise<any[]> {
  ...
  return (db as Database.Database).prepare("SELECT * FROM trades ORDER BY created_at DESC LIMIT ?").all(limit)
}
...
export async function getTradesByStrategy(strategyId: string, limit = 50): Promise<any[]> {
  ...
  return (db as Database.Database).prepare("SELECT * FROM trades WHERE strategy_id = ? ORDER BY created_at DESC LIMIT ?").all(strategyId, limit)
}

export async function getStrategyConfigs(): Promise<any[]> {
  const db = getDb()
  const rows = isPostgres ? (await (db as Pool).query("SELECT * FROM strategy_configs ORDER BY id ASC")).rows : (db as Database.Database).prepare("SELECT * FROM strategy_configs ORDER BY id ASC").all()
  return rows.map(r => ({ ...r, symbols: typeof r.symbols === "string" ? JSON.parse(r.symbols) : r.symbols, params: typeof r.params === "string" ? JSON.parse(r.params) : r.params, is_active: Boolean(r.is_active), auto_execute: Boolean(r.auto_execute) }))
}

export async function updateStrategyConfig(strategyId: string, updates: any): Promise<void> {
  const db = getDb()
  const current = isPostgres ? (await (db as Pool).query("SELECT * FROM strategy_configs WHERE strategy_id = $1", [strategyId])).rows[0] : (db as Database.Database).prepare("SELECT * FROM strategy_configs WHERE strategy_id = ?").get(strategyId) as any
  if (!current) return
  ...
  if (isPostgres) {
    ...
    (db as Database.Database).prepare("UPDATE strategy_configs SET params = ?, symbols = ?, is_active = ?, updated_at = datetime('now') WHERE strategy_id = ?").run(params, symbols, active, strategyId)
  }
}
...
export async function setStrategyActive(strategyId: string, isActive: boolean): Promise<void> {
  ...
  if (isPostgres) {
    ...
    (db as Database.Database).prepare("UPDATE strategy_configs SET is_active = 0, updated_at = datetime('now')").run()
    if (isActive) (db as Database.Database).prepare("UPDATE strategy_configs SET is_active = 1, updated_at = datetime('now') WHERE strategy_id = ?").run(strategyId)
  }
}
...
export async function insertPortfolioSnapshot(snapshot: any): Promise<void> {
  ...
  if (isPostgres) {
    ...
    (db as Database.Database).prepare("INSERT INTO portfolio_snapshots (equity, cash, buying_power, portfolio_value, profit_loss, profit_loss_pct) VALUES (@equity, @cash, @buying_power, @portfolio_value, @profit_loss, @profit_loss_pct)").run(snapshot)
  }
}
...
export async function insertTradeSignal(signal: any): Promise<void> {
  ...
  if (isPostgres) {
    ...
    (db as Database.Database).prepare("INSERT INTO trade_signals (strategy_id, symbol, action, qty, reason, confidence, price_at_signal, was_executed) VALUES (@strategy_id, @symbol, @action, @qty, @reason, @confidence, @price_at_signal, @was_executed)").run({ ...signal, was_executed: signal.was_executed ? 1 : 0 })
  }
}
